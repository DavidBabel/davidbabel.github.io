...
const baseHTML = `
<div id="splash">Désolé, ce script ne fonctionne que sous Google Chrome ou Mozilla Firefox,
et pas sur Internet Explorer, car Internet Explorer c'est pour le démon.
Tu as vraiment envie de travailler avec le démon ? Si oui tu as tords.
Ferme ça tout de suite, file télécharger un vrai navigateur qui n'est pas codé avec les pieds.</div>
<div id="astuce">Aides de saisie dans l'éditeur de code : <s>ctrl + espace </s> ou <s>ctrl + n</s></div>
<div id="menu">
  <button id="reindent">Réindenter <s>(ctrl + i)</s></button>
  ...
  <button onclick="loadSavedCode();">récup code</button>
  <span id="auto_save" onclick="toggleSaveActive();">sauvegarde auto activée</span>
    ...
    <div id="terminal-content">
      <div id="prompt">
        ...
        <span class="cells" style="width: 100%;">
          <input title="Entrez vos paramètres ici" class="console" id="args" type="text" placeholder="paramètres..." spellcheck="false">
        </span>
...
const originHTML = $("body").contents().filter(function () {
  return this.nodeType === 3;
});
const baseAlgoCodeToInsert = originHTML[0].data.trim();
originHTML.remove();
...
var superalgo = (s) => {
  ...
  const forcode = 'for(var $1 = $2 ; ($2 < $3) ? $1 <= $3 : $1 >= $3; ($2 < $3) ? $1++ : $1--)';
...
(function () {
  CodeMirror.defineOption('showInvisibles', false, (cm, val, prev) => {
    let Count = 0;
    const Maximum = cm.getOption('maxInvisibles') || 25;
  ...
  function getStyle() {
    const style = [
      ...
      '  margin-left: -1px;',
      '}',
      '.cm-cm-eol + .cm-whitespace::before {',
      '  opacity: 0;',
      '}',
    ].join('');
...
const tokenWord = [];
const basicLanguageToken = codeMirrorConfig.reduce((prev, current) => {
  if (!current.ignoreForHint) {
    const elems = current.regex.source.split('|');
    elems.forEach((e) => {
      prev.push(e.trim() + ' ');
...
CodeMirror.registerHelper("hint", "algo", function (editor, options) {
  ...
  if (curWord) {
    list.push(...basicLanguageToken.filter((token) => {
      return token.includes(curWord);
    }));
    list = list.concat(snippets.filter((snip) => {
      return snip.displayText.includes(curWord);
    }));
  } else {
    list = list.concat(snippets);
    list.push(...basicLanguageToken);
  }
  return {
    list: list,
...
CodeMirror.commands.reindent = function (cm) {
  ...
  console.log(cursor);
  const currentLineLength = cm.doc.children[0].lines[cursor.line].text.length;
  reindenter();
  cm.setCursor(cursor);
  const newLineLength = cm.doc.children[0].lines[cursor.line].text.length;
  const posChange = currentLineLength - newLineLength;
  if (posChange !== 0) {
    cursor.ch -= posChange;
    cm.setCursor(cursor);
  }
